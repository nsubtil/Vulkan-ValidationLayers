...
class UberLayerOutputGenerator(OutputGenerator):
    ...
    precallvalidate_loop = "for (auto intercept : interceptor_list->object_dispatch) {"
    precallrecord_loop = precallvalidate_loop
    # TODO: This loop should run backwards!
    postcallrecord_loop = "for (auto intercept : interceptor_list->object_dispatch) {"
...
std::unordered_map<void*, validation_object*> layer_data_map;
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL GetDeviceProcAddr(VkDevice device, const char *funcName) {
    auto interceptor_list = GetLayerDataPtr(get_dispatch_key(device), layer_data_map);
    const auto &item = name_to_funcptr_map.find(funcName);
    if (item != name_to_funcptr_map.end()) {
        return reinterpret_cast<PFN_vkVoidFunction>(item->second);
    }
    auto &table = interceptor_list->device_dispatch_table;
...
VKAPI_ATTR PFN_vkVoidFunction VKAPI_CALL GetInstanceProcAddr(VkInstance instance, const char *funcName) {
    ...
    auto interceptor_list = GetLayerDataPtr(get_dispatch_key(instance), layer_data_map);
    auto &table = interceptor_list->instance_dispatch_table;
...
VKAPI_ATTR VkResult VKAPI_CALL EnumerateDeviceExtensionProperties(VkPhysicalDevice physicalDevice, const char *pLayerName,
    ...
    auto interceptor_list = GetLayerDataPtr(get_dispatch_key(physicalDevice), layer_data_map);
    return interceptor_list->instance_dispatch_table.EnumerateDeviceExtensionProperties(physicalDevice, NULL, pCount, pProperties);
}
...
VKAPI_ATTR VkResult VKAPI_CALL CreateInstance(const VkInstanceCreateInfo *pCreateInfo, const VkAllocationCallbacks *pAllocator,
    ...
    std::vector<validation_object*> local_object_dispatch;
...
VKAPI_ATTR void VKAPI_CALL DestroyInstance(VkInstance instance, const VkAllocationCallbacks *pAllocator) {
    ...
    auto interceptor_list = GetLayerDataPtr(key, layer_data_map);
    ...
    interceptor_list->instance_dispatch_table.DestroyInstance(instance, pAllocator);
    ...
    while (interceptor_list->logging_messenger.size() > 0) {
        VkDebugUtilsMessengerEXT messenger = interceptor_list->logging_messenger.back();
        layer_destroy_messenger_callback(interceptor_list->report_data, messenger, pAllocator);
        interceptor_list->logging_messenger.pop_back();
    }
    while (interceptor_list->logging_callback.size() > 0) {
        VkDebugReportCallbackEXT callback = interceptor_list->logging_callback.back();
        layer_destroy_report_callback(interceptor_list->report_data, callback, pAllocator);
        interceptor_list->logging_callback.pop_back();
    }

    layer_debug_utils_destroy_instance(interceptor_list->report_data);

    // TODO::   Need to free each of the child objects, and then the object in the map too...
...
VKAPI_ATTR void VKAPI_CALL DestroyDevice(VkDevice device, const VkAllocationCallbacks *pAllocator) {
    ...
    interceptor_list->device_dispatch_table.DestroyDevice(device, pAllocator);
...
VKAPI_ATTR VkResult VKAPI_CALL CreateDebugReportCallbackEXT(VkInstance instance,
    ...
    VkResult result = interceptor_list->instance_dispatch_table.CreateDebugReportCallbackEXT(instance, pCreateInfo, pAllocator, pCallback);
    result = layer_create_report_callback(interceptor_list->report_data, false, pCreateInfo, pAllocator, pCallback);
    """ + postcallrecord_loop + """
...
VKAPI_ATTR void VKAPI_CALL DestroyDebugReportCallbackEXT(VkInstance instance, VkDebugReportCallbackEXT callback,
    ...
    interceptor_list->instance_dispatch_table.DestroyDebugReportCallbackEXT(instance, callback, pAllocator);
    layer_destroy_report_callback(interceptor_list->report_data, callback, pAllocator);
    """ + postcallrecord_loop + """
...
VK_LAYER_EXPORT VKAPI_ATTR VkResult VKAPI_CALL vkNegotiateLoaderLayerInterfaceVersion(VkNegotiateLayerInterface *pVersionStruct) {
    ...
    def beginFile(self, genOpts):
        ...
        self.layer_factory += 'class validation_object {\n'
        ...
        self.layer_factory += '\n'
        self.layer_factory += '        std::vector<validation_object*> object_dispatch;\n'
        self.layer_factory += '        LayerObjectTypeId container_type;\n'
        self.layer_factory += '\n'
        self.layer_factory += '        // Constructor\n'
        self.layer_factory += '        validation_object(){};\n'
        self.layer_factory += '\n'
        self.layer_factory += '        std::string layer_name = "UBER";\n'
        self.layer_factory += '\n'
    ...
    def endFile(self):
        ...
        if self.header:
            ...
            self.layer_factory += '};\n\n'
            self.layer_factory += 'extern std::unordered_map<void*, validation_object*> layer_data_map;'
    ...
    def genCmd(self, cmdinfo, name, alias):
        ...
        self.appendSection('command', '    auto interceptor_list = GetLayerDataPtr(get_dispatch_key(%s), layer_data_map);' % (dispatchable_name))
        api_function_name = cmdinfo.elem.attrib.get('name')
        params = cmdinfo.elem.findall('param/name')
        paramstext = ', '.join([str(param.text) for param in params])
        API = api_function_name.replace('vk','interceptor_list->%s_dispatch_table.' % (device_or_instance),1)
